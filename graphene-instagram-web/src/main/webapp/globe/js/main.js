...
// properties for each "trade" is: e - exporter, i - importer, v - value (USD),
// wc - weapons code (see table)
var timeBins;

// contains latlon data for each country
var latlonData;

// contains above but organized as a mapped list via ['countryname'] =
// countryobject
// each country object has data like center of country in 3d space, lat lon,
...
/*
 * 930100 – military weapons, and includes some light weapons and artillery as
 * well as machine guns and assault rifles etc. 930190 – military firearms – eg
 * assault rifles, machineguns (sub, light, heavy etc), combat shotguns, machine
 * pistols etc 930200 – pistols and revolvers 930320 – Sporting shotguns
 * (anything that isn’t rated as a military item). 930330 – Sporting rifles
 * (basically anything that isn’t fully automatic). 930621 – shotgun shells
 * 930630 – small caliber ammo (anything below 14.5mm which isn’t fired from a
 * shotgun.
 */
...
// contains info about what year, what countries, categories, etc that's being
// visualized
var selectionData;
...
function start(e) {
	...
	if (!Detector.webgl) {
		...
		mapIndexedImage.src = 'globe/images/map_indexed.png';
		mapIndexedImage.onload = function() {
			mapOutlineImage = new Image();
			mapOutlineImage.src = 'globe/images/map_outline.png';
...
function initScene() {
	...
	// THREE.ImageUtils.loadTexture( 'globe/images/map_indexed.png' );
	...
	// var mapGraphic = new
	// THREE.Texture(worldCanvas);//THREE.ImageUtils.loadTexture("globe/images/map.png");
	...
	sphere = new THREE.Mesh(new THREE.SphereGeometry(100, 40, 40),
			shaderMaterial);
	...
	// -----------------------------------------------------------------------------
	// Setup our camera
	camera = new THREE.PerspectiveCamera(12, window.innerWidth
...
function animate() {
	...
	// Disallow roll for now, this is interfering with keyboard input during
	// search
	/*
	 * if(keyboard.pressed('o') && keyboard.pressed('shift') == false)
	 * camera.rotation.z -= 0.08; if(keyboard.pressed('p') &&
	 * keyboard.pressed('shift') == false) camera.rotation.z += 0.08;
	 */
	...
	if (rotateTargetX !== undefined && rotateTargetY !== undefined) {
		...
		if (Math.abs(rotateTargetX - rotateX) < 0.1
				&& Math.abs(rotateTargetY - rotateY) < 0.1) {
	...
	rotating.traverse(function(mesh) {
		if(mesh instanceof THREE.Mesh && mesh.update !== undefined) {
			mesh.update();
		}
	});
...
function highlightCountry(countries) {
	...
	// the fact that it didn't select was because bolivia shows up as an invalid
	// country due to country name mismatch
	// ...
...
function getHistoricalData(country) {
	...
	for ( var i in timeBins) {
		...
		for ( var s in yearBin) {
			...
			var relevantCategory = (countryName == exporterCountryName && $
					.inArray(categoryName, exportCategories) >= 0)
					|| (countryName == importerCountryName && $.inArray(
							categoryName, importCategories) >= 0);

			if (relevantCategory == false)
				continue;

			// ignore all unidentified country data
			if (countryData[exporterCountryName] === undefined
					|| countryData[importerCountryName] === undefined)
				continue;
...
function getPickColor() {
	...
	var mx = (mouseX + renderer.context.canvas.width / 2);// (mouseX +
	// renderer.context.canvas.width/2)
	// * 0.25;
	var my = (-mouseY + renderer.context.canvas.height / 2);// (-mouseY +
	// renderer.context.canvas.height/2)
	// * 0.25;
	mx = Math.floor(mx);
	my = Math.floor(my);
